...
public class Stage {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < l2+1; i++) {
            ...
            lastHeightA[0][i]=a2[i-1];
            dynB[0][i]=dynB[0][i-1]+Math.abs(a2[i-1]-lastHeightB[0][i-1]);
            dynA[0][i]=dynB[0][i];
        }
       /* for (int i = 1; i < l2+1; i++) {
            lastHeightA[0][i]=a2[0];

        }
        for (int i = 1; i < l1+1; i++) {
            lastHeightB[i][0]=a1[0];

        }*/
        //i pr les i premier de A1 j pr les j premier de A2
        for (int i = 1; i < l1+1; i++) {
            for (int j = 1; j < l2+1; j++) {
                int c1=dynA[i-1][j]+Math.abs(a1[i-1]-lastHeightA[i-1][j]);
                int c2=dynB[i-1][j]+Math.abs(a1[i-1]-a2[j-1]);
                if(c1>c2){
                    c1=c2;
                }
                dynA[i][j]=c1;
                ...
                c1=dynA[i][j-1]+Math.abs(a2[j-1]-a1[i-1]);
